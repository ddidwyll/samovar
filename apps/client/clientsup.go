...
func factory_ClientSup() gen.ProcessBehavior {
	return &ClientSup{}
}

type ClientSup struct {
	act.Supervisor
}

// Init invoked on a spawn Supervisor process. This is a mandatory callback for the implementation
func (sup *ClientSup) Init(args ...any) (act.SupervisorSpec, error) {
	...
	spec.Children = []act.SupervisorChildSpec{
		{
			Name:    "clientactor",
			Factory: factory_ClientActor,
		},
		{
			Name:    "clientserver",
			Factory: factory_ClientServer,
		},
	}
...
func (sup *ClientSup) HandleChildStart(name gen.Atom, pid gen.PID) error {
	return nil
}

// HandleChildTerminate invoked on a child process termination if option EnableHandleChild
// was enabled in act.SupervisorSpec
func (sup *ClientSup) HandleChildTerminate(name gen.Atom, pid gen.PID, reason error) error {
	return nil
}
...
func (sup *ClientSup) HandleMessage(from gen.PID, message any) error {
...
func (sup *ClientSup) HandleCall(from gen.PID, ref gen.Ref, request any) (any, error) {
...
func (sup *ClientSup) Terminate(reason error) {
	sup.Log().Info("supervisor terminated with reason: %s", reason)
}

// HandleInspect invoked on the request made with gen.Process.Inspect(...)
func (sup *ClientSup) HandleInspect(from gen.PID, item ...string) map[string]string {
